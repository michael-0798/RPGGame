...
/**
 * RedDragon is a subclass of Characer, it inherites all the fields from the 
 ...
 * @since Mar 1, 2021
 */
public class RedDragon extends Character{
	 ...
	 * maxHealth will be 1 to 400 (inclusive, D20*D20), strength will
	 * be 2 to 40 (inclusive, 2*D20). The current health is set to be the
	 * same as the maxHealth.
	 * @see DiceBag
	 */
	public RedDragon() {
		maxHealth =DiceBag.roll(Dice.TWENTY)*DiceBag.roll(Dice.TWENTY); 
		currentHealth = maxHealth;
		strength = 2*DiceBag.roll(Dice.TWENTY);
	}
	 ...
	 */
	public RedDragon(int maxHealth, int currentHealth, int strength) {
		super(maxHealth, currentHealth,strength);
	}
	
	/**
	 * attack() method is overrided by the class RedDragon,
	 * it will return subclass of the class Attack, in this case, there are 
	 * 70% chances to return MagicAttack's object, and 30% to return FireAttack;
	 * both MagicAttack's and FireAttack's damages are: 3 * D20,
	 */
	@Override
	public Attack attack() {
		int chance =DiceBag.roll(Dice.HUNDRED); 
		if(chance>0 && chance<=70 ) {
			MagicAttack att = new MagicAttack(3*DiceBag.roll(Dice.TWENTY));
			return att;
		}else {
			FireAttack att = new FireAttack(3*DiceBag.roll(Dice.TWENTY));
			return att;
		}
	}
	 ...
	 * in this case, if the attack is frost attack, then the damage value will be 
	 * double; if it's a fire attack, there will be no damage caused; if magic 
